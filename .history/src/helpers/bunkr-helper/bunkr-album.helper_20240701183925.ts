...
export class BunkrAlbumHelper {
  ...
  async fetchAll(
    ...
    const videoIds = await Promise.all(
      videos.map(video => this.parseVideoUrl(video.videoUrl)),
    );

    const downloadUrls = Promise.all(
      videoIds.map(videoid => this.parseVideoIdUrl(videoid)),
    );
  ...
  async parseVideoUrl(url: string): Promise<string | undefined> {
    //#czmDownloadz
    const $ = await getCheerio(url);
    const videoIdUrl = $('#czmDownloadz').attr('href');

    return videoIdUrl!;
  }

  async parseVideoIdUrl(url: string): Promise<string> {
    //#czmDownloadz
    const $ = await getCheerio(url);
    const videoIdUrl = $('a.btn.btn-main.btn-lg.rounded-full').attr('href');
    console.log(videoIdUrl);
    return '';
  }
