...
export class PhHelper {
  ...
  async getVideoCounter(url: string): Promise<string> {
    ...
    const counter = $('div.showingCounter').text().trim();
    const split = counter.split(' of ');
    console.log(split[1]);

    return '';
  }

  async getVideosDetail(urls: string[]): Promise<PhVideo[]> {
    console.log('Started');
    const result = await Promise.all(urls.map(url => this.getVideo(url)));
    console.log('end');
    return result;
  }
  ...
  async getVideo(url: string): Promise<PhVideo> {
    ...
    return {
      ...
      url: mediaDefinition?.videoUrl?.replace(/\\\//g, '/'),
    };
  }
